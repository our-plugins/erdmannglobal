<x-admin-layout>
    <x-slot name="title">Add Blog Post</x-slot>
    ...
    <div class="max-w-4xl">
        ...
        <form action="{{ route('admin.blog.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                    <div>
                        ...
                        <textarea name="excerpt_en" rows="3"
                            class="w-full rounded-lg border-gray-300 focus:border-gold-500 focus:ring-gold-500">{{ old('excerpt_en') }}</textarea>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Excerpt (French)</label>
                        <textarea name="excerpt_fr" rows="3"
                            class="w-full rounded-lg border-gray-300 focus:border-gold-500 focus:ring-gold-500">{{ old('excerpt_fr') }}</textarea>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                    <div>
                        ...
                        <textarea name="content_en" rows="10" required
                            class="w-full rounded-lg border-gray-300 focus:border-gold-500 focus:ring-gold-500">{{ old('content_en') }}</textarea>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Content (French) *</label>
                        <textarea name="content_fr" rows="10" required
                            class="w-full rounded-lg border-gray-300 focus:border-gold-500 focus:ring-gold-500">{{ old('content_fr') }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="author_name" value="{{ old('author_name') }}" placeholder="Enter author name"
                            class="w-full rounded-lg border-gray-300 focus:border-gold-500 focus:ring-gold-500">
                    </div>
                    <div>
            ...
            <div class="flex justify-end gap-4">
                ...
                <button type="submit" class="bg-gold-500 hover:bg-gold-600 text-white font-semibold px-6 py-3 rounded-lg">Create Post</button>
            </div>
        </form>
    </div>
</x-admin-layout>
